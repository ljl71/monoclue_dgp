...
class DDNLoss(nn.Module):
    ...
    def forward(self, depth_logits, gt_boxes2d, num_gt_per_img, gt_center_depth, gt_sam_depth=None):
        """
        Gets depth_map loss
        Args:
        ...
        # Bin depth map to create target. SAM depth supports the foreground-depth
        # ablation; box depth keeps the original dense box-level supervision.
        if gt_sam_depth is None:
