...
class Det2DTransformer(nn.Module):
    def __init__(
        ...
        self.use_query_initializer = use_query_initializer
        self.query_init = QueryInitializer(d_model, query_initializer_cfg) if use_query_initializer else None
    ...
    def forward(self, srcs, masks, pos_embeds, region_mask, query_embed=None, return_debug=False):
        ...
        query_init_debug = None
        if self.query_init is not None:
            if return_debug:
                tgt, memory, query_init_debug = self.query_init(
                    memory, region_mask, tgt, deform_info, return_debug=True)
            else:
        ...
        if return_debug:
            intermediate_output['query_initializer_debug'] = query_init_debug
...
def build_det2d_transformer(cfg):
    return Det2DTransformer(
        ...
        group_num=cfg.get('group_num', 1),
        use_query_initializer=cfg.get('use_query_initializer', True),
        query_initializer_cfg=cfg.get('query_initializer', None))
