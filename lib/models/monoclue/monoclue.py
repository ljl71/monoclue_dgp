...
class MonoCLUE(nn.Module):
    ...
    def __init__(self, backbone, depth_predictor, det2d_transformer, det3d_transformer,
                 ...
                 region_attention='eca', use_fem=True):
        ...
        self.region_head = RegionSegHead(
            d_model=hidden_dim,
            attention_type=region_attention,
        )
        ...
        self.fem_layers = nn.ModuleList([FEM(hidden_dim) for _ in range(num_feature_levels)]) if use_fem else nn.ModuleList()
    ...
    def forward(self, images, calibs, img_sizes, dn_args=None, return_visual_debug=False):
        ...
        intermediate_output = self.det2d_transformer(
            srcs, masks, pos, seg_embed, query_embeds, return_debug=return_visual_debug)
        ...
        if return_visual_debug:
            out['visual_debug'] = {
                'weighted_depth': weighted_depth.detach(),
                'seg_embed': [x.detach() for x in seg_embed],
                'query_initializer': intermediate_output.get('query_initializer_debug', None),
            }
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, focal_alpha, losses, inter_losses, group_num=11,
                 depth_map_target='sam', region_target='sam'):
        ...
        self.depth_map_target = depth_map_target
    ...
    def loss_depth_map(self, outputs, targets, indices, num_boxes):
        ...
        gt_boxes2d = torch.cat([t['boxes'] for t in targets], dim=0) * torch.tensor([80, 24, 80, 24], device=device)
        gt_boxes2d = box_ops.box_cxcywh_to_xyxy(gt_boxes2d)
        gt_center_depth = torch.cat([t['depth'] for t in targets], dim=0).squeeze(dim=1)

        if self.depth_map_target == 'sam':
            gt_depth_map = torch.cat([t['depth_map'].unsqueeze(0) for t in targets], dim=0)
            gt_depth_map = F.interpolate(gt_depth_map.unsqueeze(1).float(), size=depth_map_logits.shape[2:],
                                         mode='nearest')
            gt_depth_map = gt_depth_map.squeeze(1)
        elif self.depth_map_target == 'box':
            gt_depth_map = None
        else:
            raise ValueError(f"Unsupported depth map target: {self.depth_map_target}")

        losses = dict()

        losses["loss_depth_map"] = self.ddn_loss(
            depth_map_logits, gt_boxes2d, num_gt_per_img, gt_center_depth, gt_depth_map)
        return losses

    def loss_region(self, outputs, targets, indices, num_boxes):
        region_probs = outputs['pred_region_prob']
        if self.region_target == 'sam':
            gt_region = torch.cat([t['sam_region'].unsqueeze(0) for t in targets], dim=0)
        elif self.region_target == 'box':
            gt_region = torch.cat([t['obj_region'].unsqueeze(0) for t in targets], dim=0)
        else:
            raise ValueError(f"Unsupported region target: {self.region_target}")
...
def build(cfg):
    ...
    # loss
    depth_map_target = cfg.get('depth_map_target', cfg.get('depth_map_supervision', 'sam'))
    region_target = cfg.get('region_target', cfg.get('region_supervision', 'sam'))
    use_depth_map_loss = cfg.get('depth_map_loss_coef', 0) > 0 and depth_map_target != 'none'
    use_region_loss = cfg.get('region_loss_coef', 0) > 0 and region_target != 'none'
    ...
    criterion = SetCriterion(
        ...
        group_num=cfg['group_num'],
        depth_map_target=depth_map_target,
