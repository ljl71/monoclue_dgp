    ...
    if os.path.isfile(filename):
        ...
        if model is not None and checkpoint['model_state'] is not None:
            ...
            if not strict and logger is not None:
                logger.info("==> Non-strict load. Missing keys: {}".format(incompatible.missing_keys))
                logger.info("==> Non-strict load. Unexpected keys: {}".format(incompatible.unexpected_keys))
